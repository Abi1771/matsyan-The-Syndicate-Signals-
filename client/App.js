...
function AppRoutes({ user, setUser, showSignUp, setShowSignUp, snackbar, setSnackbar, catches, setCatches }) {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (user.role === 'Customer' && !currentPath.startsWith('/customer')) {
        console.log('Redirecting customer to /customer');
        navigate('/customer');
      } else if (user.role === 'Fisherman' && !currentPath.startsWith('/fisherman')) {
        console.log('Redirecting fisherman to /fisherman');
        navigate('/fisherman');
      } else if (user.role === 'Inspector' && !currentPath.startsWith('/inspector')) {
        console.log('Redirecting inspector to /inspector');
        navigate('/inspector');
      }
    }
  }, [user, navigate]);
    ...
    <>
      {(!user) ? (
        ...
        <Routes>
          ...
          {user.role === 'Fisherman' && (
            <>
              {console.log('Rendering Fisherman routes')}
          ...
          {user.role === 'Inspector' && (
            <>
              {console.log('Rendering Inspector routes')}
